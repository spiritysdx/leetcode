...
list.Element 是 Go container/list 包中用于表示双向链表节点的结构体。它包含了节点存储的值、指向前后节点的指针，以及它所属的链表。通过 list.Element，你可以实现对链表中每个节点的访问、移动、删除等操作。
   使用的时候需要断言为对应的类型，所以需要写出entry结构体存入键值对
*/
